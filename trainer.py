...
class TrainerBase:
    def __init__(self, configs):
        ...
        # finetune
        if 'finetune' in self.configs.keys() and self.configs['finetune']:
            self.finetune_from_ckpt()
        
        if 'gan_opt' in self.configs.train.keys():
            self.gan_loss = True
            self.cri_gan = GANLoss(self.configs.train.gan_opt.gan_type, real_label_val=self.configs.train.gan_opt.real_label_val, 
                                   fake_label_val=self.configs.train.gan_opt.fake_label_val, loss_weight=self.configs.train.gan_opt.loss_weight)
            self.net_d_iters = self.configs.train.net_d_iters
            self.net_d_init_iters = self.configs.train.net_d_init_iters
        else:
            self.gan_loss = False
    ...
    def finetune_from_ckpt(self):
        ckpt_path = self.configs['finetune_ckpt']
        ckpt = torch.load(ckpt_path, map_location=f"cuda:{self.rank}")
    ...
    def setup_optimizaton(self):
        ...
        if 'network_d' in self.configs.model.keys():
            self.optimizer_d = torch.optim.AdamW(self.network_d.parameters(),
                                    lr=self.configs.train.lr,
                                    weight_decay=self.configs.train.weight_decay)
    ...
    def build_model(self):
        ...
        if 'network_d' in self.configs.model.keys():
            network_d = UNetDiscriminatorSN(self.configs.model.network_d.num_in_ch, num_feat=self.configs.model.network_d.num_feat, 
                                                skip_connection=self.configs.model.network_d.skip_connection)
            if self.num_gpus > 1:
                self.network_d = DDP(network_d.cuda(), device_ids=[self.rank,])  # wrap the network
            else:
                self.network_d = network_d.cuda()

    def build_dataloader(self, need_shuffle=True):
        ...
        if 'gan_clean' in self.configs.data.keys():
            self.separate_gan_data = True
            gan_clean_dataset_config = self.configs.data.get('gan_clean', dict)
            datasets['gan'] = create_dataset(gan_clean_dataset_config)
            # gan clean data dataloader
            if self.rank == 0:
                gan_data_length = len(datasets['gan'])
                self.logger.info('Number of images in gan data set: {:d}'.format(gan_data_length))
            if self.num_gpus > 1:
                gan_shuffle = False
                gan_sampler = udata.distributed.DistributedSampler(datasets['gan'],
                                                            num_replicas=self.num_gpus,
                                                            rank=self.rank)
            else:
                gan_shuffle = True
                gan_sampler = None
            dataloaders['gan'] = _wrap_loader(udata.DataLoader(
                                        datasets['gan'],
                                        batch_size=self.configs.train.batch[0] // self.num_gpus,
                                        shuffle=gan_shuffle,
                                        drop_last=False,
            ...
            self.merge_gan_data_finetune = self.configs.data.gan_clean.merge_gan_data_finetune
        
        else:
            self.separate_gan_data = False
    ...
    def train(self):
        ...
        for ii in range(self.iters_start, self.configs.train.iterations):
            ...
            # prepare gan data
            if self.gan_loss and self.separate_gan_data:
                gan_data = self.prepare_data(
                        next(self.dataloaders['gan']),
                        self.configs.data.train.type.lower() == 'realesrgan',
                        )
            else:
                gan_data = None
            ...
            # training phase
            self.training_step(data, gan_data=gan_data)
        ...
        self.save_ckpt_final()

    def save_finetune_data(self, lq_dir, gt_dir, num_imgs):
        ...
        self.build_dataloader(need_shuffle=False) # prepare data: self.dataloaders, self.datasets, self.sampler
        ...
        for ii in range(self.iters_start, self.configs.train.iterations):
            ...
            # save data to the corresponding directory
            lq = data['lq']
            gt = data['gt']
            gt_path = data['gt_path'][0]
            saved_name = Path(gt_path).stem
            
            torchvision.utils.save_image(lq, os.path.join(lq_dir, saved_name + '.png'))
            torchvision.utils.save_image(gt, os.path.join(gt_dir, saved_name + '.png'))
            
            img_cnt += 1
            if img_cnt >= num_imgs:
                break

    def training_step(self, data, gan_data=None):
        pass
    ...
    def save_ckpt(self):
        ...
        # save the checkpoint that can be directly loaded by the inference script
        ckpt_only_path = self.ckpt_dir / 'model_only_{:d}.pth'.format(self.current_iters)
        torch.save(self.model.state_dict(), ckpt_only_path)
        
        self.logger.info("Save network ckpt: " + ckpt_only_path.name)

        if 'network_d' in self.configs.model.keys():
            # save the checkpoint that can be directly loaded by the inference script
            ckpt_only_path_network_d = self.ckpt_dir / 'network_d_{:d}.pth'.format(self.current_iters)
            torch.save(self.network_d.state_dict(), ckpt_only_path_network_d)
            
            self.logger.info("Save discriminator ckpt: " + ckpt_only_path_network_d.name)

    def save_ckpt_final(self):        
        # save the checkpoint that can be directly loaded by the inference script
        ckpt_only_path = self.ckpt_dir / 'model_latest.pth'
        torch.save(self.model.state_dict(), ckpt_only_path)
        
        self.logger.info("Save latest network ckpt: " + ckpt_only_path.name)

        if 'network_d' in self.configs.model.keys():
            # save the checkpoint that can be directly loaded by the inference script
            ckpt_only_path_network_d = self.ckpt_dir / 'network_d_latest.pth'
            torch.save(self.network_d.state_dict(), ckpt_only_path_network_d)
            
            self.logger.info("Save latest discriminator ckpt: " + ckpt_only_path_network_d.name)

class TrainerSR(TrainerBase):
    def __init__(self, configs):
        super().__init__(configs)

    def loss_fun_l2(self, pred, target, reduction='sum'):
        return F.mse_loss(pred, target, reduction=reduction)

    def loss_fun_l1(self, pred, target, reduction='sum'):
        return F.l1_loss(pred, target, reduction=reduction)
    
    def compute_loss(self, pred, target, num_samples, return_dict=False):
        loss = 0
        reduction = self.configs.train.loss.reduction
        if return_dict:
            loss_dict = {}
        if reduction == 'mean':
            if self.configs.train.loss.l1_loss != 0.0:
                loss_l1 = self.loss_fun_l1(pred, target, reduction=reduction) * self.configs.train.loss.l1_loss
                if return_dict:
                    loss_dict['L1'] = loss_l1.clone().detach()
                loss += loss_l1
            if self.configs.train.loss.l2_loss != 0.0:
                loss_l2 = self.loss_fun_l2(pred, target, reduction=reduction) * self.configs.train.loss.l2_loss
                if return_dict:
                    loss_dict['L2'] = loss_l2.clone().detach()
                loss += loss_l2
        elif reduction == 'sum':
            if self.configs.train.loss.l1_loss != 0.0:
                loss += self.loss_fun_l1(pred, target, reduction=reduction) * self.configs.train.loss.l1_loss / num_samples
            if self.configs.train.loss.l2_loss != 0.0:
                loss += self.loss_fun_l2(pred, target, reduction=reduction) * self.configs.train.loss.l2_loss / num_samples
        else:
            raise NotImplementedError
        if self.configs.train.loss.lpips != 0.0:
            lpips_loss = self.lpips_loss(pred, target, normalize=True).sum() * self.configs.train.loss.lpips
            if return_dict:
    ...
    def prepare_data(self, data, real_esrgan=True):
        if real_esrgan:
            ...
            returned_dict = {'lq': data['lq'].cuda(), 'gt': data['gt'].cuda()}
            if 'gt_path' in data.keys():
                returned_dict['gt_path'] =  data['gt_path']
    ...
    def setup_optimizaton(self):
        ...
        # if 'network_d' in self.configs.model.keys():
        #     self.lr_sheduler_d = torch.optim.lr_scheduler.CosineAnnealingLR(
        #             self.optimizer_d,
        #             T_max = self.configs.train.iterations,
        #             eta_min=self.configs.train.lr_min,
        #             )            
        ...
        if self.gan_loss:
            ...
            for jj in range(0, current_batchsize, micro_batchsize):
                ...
                if gan_data is not None:
                    micro_gan_data = {key:value[jj:jj+micro_batchsize,] for key, value in gan_data.items()}
                    loss_gan_data = micro_gan_data['gt']
                    if self.merge_gan_data_finetune:
                        data_length = loss_gan_data.shape[0] // 2
                        mixed_loss_gan_data = torch.cat([loss_gan_data[:data_length], micro_data['gt'][data_length:].clone().detach()])
                        loss_gan_data = mixed_loss_gan_data[torch.randperm(mixed_loss_gan_data.shape[0]),:]
                else:
                    loss_gan_data = micro_data['gt'].clone().detach()
                
                if (self.current_iters % self.net_d_iters == 0 and self.current_iters > self.net_d_init_iters):
                    if last_batch or self.num_gpus <= 1:
                        if 'loss' in self.configs.train.keys():
                            loss_dict, loss = self.compute_loss(hq_pred, micro_data['gt'], hq_pred.shape[0], return_dict=True)
                        else:
                            loss = self.loss_fun_l2(hq_pred, micro_data['gt']) / hq_pred.shape[0]
                        # loss = self.compute_loss(hq_pred, micro_data['gt'], hq_pred.shape[0])
                    else:
                    ...
                    # gan loss
                    fake_g_pred = self.network_d(hq_pred)
                    l_g_gan = self.cri_gan(fake_g_pred, True, is_disc=False)
                    
                    loss_dict['gan'] = l_g_gan.clone().detach()
                ...
                # real
                real_d_pred = self.network_d(loss_gan_data)
                l_d_real = self.cri_gan(real_d_pred, True, is_disc=True)
                
                loss_dict['l_d_real'] = l_d_real.clone().detach()
                
                l_d_real /= num_grad_accumulate
                l_d_real.backward()
                
                # fake
                fake_d_pred = self.network_d(hq_pred.detach().clone())  # clone for pt1.9
                l_d_fake = self.cri_gan(fake_d_pred, False, is_disc=True)
                
                loss_dict['l_d_fake'] = l_d_fake.clone().detach()
                
                l_d_fake /= num_grad_accumulate
                l_d_fake.backward()

                # make logging
                self.log_step_train_dict(hq_pred, loss_dict, micro_data, flag=last_batch)
    ...
    def log_step_train_dict(self, hq_pred, loss_dict, batch, flag=False, phase='train'):
        '''
        param loss: loss value
        '''
        if self.rank == 0:
            if self.current_iters % self.configs.train.log_freq[0] == 0 and flag:
                log_str = 'Train:{:05d}/{:05d}, Loss:{}, lr:{:.2e}'.format(
                ...
                for loss_key in loss_dict.keys():
                    self.writer.add_scalar(loss_key, loss_dict[loss_key], self.log_step[phase])
    ...
    def validation(self, phase='val'):
        if self.rank == 0:
            ...
            # tensorboard
            self.writer.add_scalar('Validation PSNR', psnr_mean, self.log_step[phase])
    ...
    def build_dataloader(self, need_shuffle=True):
        super().build_dataloader(need_shuffle=need_shuffle)
