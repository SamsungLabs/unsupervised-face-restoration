...
__all__ = ["BAYER_MODE", "XTRANS_MODE", "BAYER_TETRA_MODE", "Dataset",
...
BAYER_TETRA_MODE = "bayer_tetra"
"""Applies a Bayer tetra mosaic pattern."""
...
class Dataset(TorchDataset):
    ...
    def __init__(self, root, download=False,
        ...
        if mode not in [BAYER_MODE, BAYER_TETRA_MODE, XTRANS_MODE]:
            raise ValueError("Dataset mode should be '%s, '%s' or '%s', got"
                             " %s" % (BAYER_MODE, BAYER_TETRA_MODE, XTRANS_MODE, mode))
        self.mode = mode
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == BAYER_MODE:
            mosaic = bayer(img)
        elif self.mode == BAYER_TETRA_MODE:
            mosaic = bayer(img, tetra=True)
        else:
            mosaic = xtrans(img)
