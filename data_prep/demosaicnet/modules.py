...
_BAYER_TETRA_WEIGHTS = resource_filename(__name__, 'data/bayer_tetra.pth')
...
class BayerDemosaick(nn.Module):
  ...
  models from scratch.
  """
  def __init__(self, depth=15, width=64, pretrained=True, pad=False, tetra=False):
    ...
    if pretrained:
      ...
      if tetra:
        state_dict = th.load(_BAYER_TETRA_WEIGHTS, map_location=th.device('cpu'))
      else:
