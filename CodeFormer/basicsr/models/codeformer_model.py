...
class CodeFormerModel(SRModel):
    ...
    def init_training_settings(self):
        ...
        if self.opt.get('network_vqgan', None) is not None and self.opt['datasets'].get('latent_gt_path') is None:
            ...
            # load the pre-trained ckpt instead, and freeze all parameters
            self.hq_vqgan_fix = build_network(self.opt['network_g']).to(self.device)
            load_path = self.opt['path'].get('pretrain_network_g', None)
    ...
    def setup_optimizers(self):
        ...
        # optimizer d
        if self.fidelity_weight > 0:
            self.freeze_discriminator = train_opt.get('freeze_discriminator', True)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.fidelity_weight > 0:
            ...
            # self.output, logits, lq_feat = self.net_g(self.input, w=self.fidelity_weight, detach_16=False)
        else:
            logits, lq_feat = self.net_g(self.input, w=0, code_only=True)
            
            # for debug purpose
            # out, logits, lq_feat = self.net_g(self.input, w=0, code_only=False)
        ...
        # optimize net_d
        if  current_iter > self.net_d_start_iter and self.fidelity_weight > 0 and not self.freeze_discriminator:
            for p in self.net_d.parameters():
